...
export const addLeadsToCampaign = async (campaignId: string, leads: any[]) => {
  try {
    ...
    console.log({ newLeadsCount, newRemaining });
...
export const fetchLeadsForCampaign = async (
  ...
  try {
    console.log({ campaignId, startIndex, endIndex });
...
export const updateCampaignStats = async (campaignId: string) => {
  try {
    ...
    console.log("updateCampaignStats called for campaign:", campaignId);
    console.log(
      "Note: Campaign stats are now updated automatically via database triggers"
    );
...
export const getTotalPagesForCampaign = async (
  campaignId: string,
  itemsPerPage: number = 50
): Promise<{ totalPages: number; totalLeads: number }> => {
  ...
  if (error) {
    console.error("Error getting lead count for campaign:", error);
    return {
      totalPages: 1,
      totalLeads: 0,
    };
